...
{
	std::unique_ptr<Force> gravityForce = std::make_unique<Aguamenti::GravityForce>(gravity);
	m_Forces.push_back(std::move(gravityForce));
}

void PhysicsScene::AddDragForce(const Aguamenti::Real k1, const Aguamenti::Real k2)
{
	std::unique_ptr<Force> dragForce = std::make_unique<Aguamenti::DragForce>(k1, k2);
